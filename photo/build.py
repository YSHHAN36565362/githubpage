...
하는 일
  ...
  6. 이력서 페이지의 Hobby 미리보기가 읽을 경량 data/preview.js 를 씁니다
...
def main():
    ...
    # ── 이력서 페이지(../index.html)의 Hobby 섹션용 경량 데이터 ──────────
    # trips.js 는 사진이 늘어나면 EXIF 까지 들어가 수백 KB 가 됩니다.
    # 이력서 첫 화면에서 그걸 통째로 받게 하지 않으려고, 표지와 제목만 담은
    # 작은 파일을 따로 씁니다. 경로는 저장소 루트(../) 기준으로 씁니다.
    preview = {
        ...
        "trips": [{
            ...
            "cover": "photo/" + t["cover"],
            "count": t["count"],
            "cities": len(t.get("spots", [])),
            "hasPhotos": t["hasPhotos"],
        } for t in trips_out],
    }
    with open(os.path.join(DATA, "preview.js"), "w", encoding="utf-8") as f:
        f.write("/* build.py 가 자동 생성합니다. 직접 고치지 마세요. */\n")
        f.write("/* 저장소 루트의 index.html(이력서) Hobby 섹션이 읽습니다. */\n")
        f.write("window.SHUTTERLOG_PREVIEW = ")
