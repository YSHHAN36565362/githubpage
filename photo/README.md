...
- 공개 주소: https://yshhan36565362.github.io/githubpage/photo/
- 메인 포트폴리오(이력서): https://yshhan36565362.github.io/githubpage/
- 2026-08 부터 `githubpage` 저장소 안의 `photo/` 폴더로 합쳤습니다.
  (이전 위치: 별도 저장소 `shutterlog`)
...
```
photos/
  2024-01_tokyo/        2024.01.22–26   도쿄
  2024-01_taipei/       2024.01.28–31   타이베이
  2024-06_hongkong/     2024.06.15–18   홍콩
  2024-06_sapporo/      2024.06.21–25   삿포로
  2025-01_tokyo/        2025.01.17–24   도쿄
  2025-06_kansai/       2025.06         간사이 (KIX)
  2025-08_europe/       2025.08.19–09.06 유럽 8개국
  2025-12_hongkong/     2025.12         홍콩
  2026-01_kanto/        2026.01         간토
```
...
| 슬러그는 영문 소문자 + `-` | URL 에 그대로 들어갑니다 (`trip.html?id=2025-06_kansai`) |
...
> ### 원본은 저장소에 올리지 않습니다 (용량 정책)
>
> 정리된 하루치 폴더가 25장에 300MB 수준입니다. 원본을 그대로 커밋하면
> GitHub Pages 권장 용량(1GB)을 여행 서너 건 만에 넘깁니다.
>
> 그래서 `photos/` 는 저장소 루트의 `.gitignore` 로 **통째로 제외**되어 있습니다.
> 원본은 맥북에만 두시고, 커밋되는 것은 `build.py` 가 만든 웹용 사본뿐입니다.
>
> | 생성물 | 크기 | 쓰이는 곳 |
> |---|---|---|
> | `thumbs/<폴더>/<이름>_600.jpg`  | 장당 약 40–80KB   | 갤러리 타일 · 지구본 마커 · 이력서 미리보기 |
> | `thumbs/<폴더>/<이름>_1600.jpg` | 장당 약 200–400KB | 라이트박스 확대 |
>
> 여행 9건 × 15장 ≈ 135장 ≈ **40MB** 입니다. 여유가 아주 많습니다.
> 웹에서 보는 용도로는 1600px 이면 충분하고, 그 이상은 화면에서 차이가 나지 않으면서
> 로딩만 느려집니다.
>
> 다른 컴퓨터에서 `build.py` 를 다시 돌릴 수는 없게 되지만, 사이트는 `thumbs/` 만
> 있으면 완전히 동작합니다.
